...
interface GenerateDocumentsClientProps {
  templateId: string;
...
export function GenerateDocumentsClient({
  ...
  const handleDownloadTemplate = () => {
    try {
      window.location.href = `/api/templates/${templateId}/excel-template`;
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fetch(`/api/templates/${templateId}/batch-generate`, {
        method: "POST",
        body: formData,
      });
